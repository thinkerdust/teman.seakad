    ...
    >
        ...
        <div id="invitations-table-container">
            <x-admin.card>
                <x-slot:header>
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            @if(isset($isRegularUser) && $isRegularUser)
                                <span class="inline-flex items-center rounded-md bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 ring-1 ring-inset ring-indigo-700/10 dark:bg-indigo-950/20 dark:text-indigo-400">
                                    Kuota: {{ $createdCount }} / {{ $totalQuota }}
                                </span>
                            @endif
                        </div>
                        
                        @if(auth()->user()->hasPermission('invitation.create'))
                            @if(isset($isRegularUser) && $isRegularUser && $remainingQuota <= 0)
                                <button 
                                    disabled
                                    class="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600"
                                    title="Kuota pembuatan undangan Anda sudah habis"
                                >
                                    <svg class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                                    </svg>
                                    Buat Undangan (Kuota Habis)
                                </button>
                            @else
                                <button 
                ...
                @if(isset($isRegularUser) && $isRegularUser && $remainingQuota <= 0)
                    <div class="mb-6 rounded-xl bg-rose-50 p-4 text-sm text-rose-800 dark:bg-rose-950/20 dark:text-rose-400 border border-rose-100 dark:border-rose-950/30 flex items-center gap-3">
                        ...
                        <div>
                            <span class="font-semibold">Kuota pembuatan undangan Anda sudah habis.</span> Silakan lakukan perpanjangan langganan atau hubungi administrator untuk melakukan penambahan kuota.
                        </div>
                    </div>
                @endif
