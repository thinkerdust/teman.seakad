<section class="events-section py-16 px-4 border-b border-stone-200/50" data-animation="fade-up">
        ...
        @if(count($invitationData['events']) > 0)
            <div class="space-y-6 max-w-md mx-auto">
                @foreach($invitationData['events'] as $event)
                    <div class="event-card bg-white p-6 sm:p-8 rounded-2xl border border-stone-200/80 shadow-sm flex flex-col justify-between relative overflow-hidden text-left">
                        <div class="relative z-10 space-y-6">
                            <div class="text-center">
                                <h4 class="event-name text-xl sm:text-2xl font-bold">
                                    {{ $event['name'] }}
                                </h4>
                                <div class="h-[1px] w-12 bg-current opacity-20 mx-auto my-3 event-card-divider"></div>
                            </div>

                            <div class="space-y-4">
                                <!-- Tanggal -->
                                <div class="flex items-start gap-3">
                                    <div class="mt-0.5 p-1.5 rounded-lg event-icon-bg">
                        ...
                        @if($invitationData['maps_url'])
                            <div class="mt-6 pt-4 border-t border-stone-100 event-card-footer">
                                <a 
                                    href="{{ $invitationData['maps_url'] }}" 
                                    target="_blank" 
                                    class="w-full inline-flex items-center justify-center gap-2 rounded-xl text-white text-xs sm:text-sm font-semibold py-2.5 px-4 shadow transition duration-200 event-map-btn"
                                >
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
